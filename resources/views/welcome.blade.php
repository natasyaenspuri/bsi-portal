...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'BSI Portal') }}</title>
        <!-- Fonts -->
                ...
                <div class="flex justify-between h-16">
                    ...
                    <div class="flex items-center">
                        @if (Route::has('login'))
                            <div class="space-x-4">
                                @auth
                                    <a href="{{ url('/dashboard') }}" class="text-[#00A39D] hover:text-[#008f8a] font-semibold">Dashboard</a>
                                @else
                                    <a href="{{ route('login') }}" class="text-gray-600 hover:text-[#00A39D] font-medium transition">Log in</a>
                                    <a href="{{ route('register') }}" class="bg-[#00A39D] hover:bg-[#008f8a] text-white px-4 py-2 rounded-lg font-medium transition shadow-md hover:shadow-lg">Register</a>
                                @endauth
                            </div>
                        @endif
                    </div>
            ...
            <div class="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-center gap-12">
                <div class="max-w-2xl">
                    ...
                    <div class="flex gap-4">
                        <a href="{{ route('register') }}" class="bg-[#F39C12] text-white px-8 py-3 rounded-lg font-bold text-lg hover:bg-yellow-600 transition shadow-lg transform hover:-translate-y-1">
        ...
        <section class="bg-[#00A39D] text-white py-16">
            <div class="max-w-4xl mx-auto text-center px-4">
                ...
                <div class="flex justify-center gap-4">
                    <a href="{{ route('register') }}" class="bg-white text-[#00A39D] px-8 py-3 rounded-lg font-bold hover:bg-gray-100 transition shadow-lg">
                        Daftar Sekarang
                    </a>
                </div>
            </div>
        ...
        <footer class="bg-gray-900 text-white py-12 border-t border-gray-800">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-6">
                ...
                <div class="text-gray-500 text-sm">
                    &copy; {{ date('Y') }} PT Bank Syariah Indonesia Tbk. All rights reserved. <br>
